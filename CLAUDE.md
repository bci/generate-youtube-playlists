...
For project-specific context — what this tool does, layout, commands, conventions, and guardrails — see **[AGENTS.md](AGENTS.md)**, imported below.
...
Every write here lands on a real person's YouTube account, and every call is drawn from a
hard daily budget. Both are easy to spend and impossible to un-spend.

**Know what a call costs before you add one.** The YouTube Data API gives 10,000 units/day
by default. `playlistItems.insert`, `playlistItems.delete` and `videos.rate` are **50 units
each** — about 200 writes a day, total, across every channel. `list` calls are 1 unit;
`search.list` is **100**. A loop that looks harmless in review can drain a day's budget in
seconds, and the reset is ~midnight Pacific, not on demand.

**Never write a retry loop around a write.** `withRetry` exists for transient network
aborts. Quota errors and permanent 400s are declared fatal on purpose: retrying them burns
50 units per attempt and cannot succeed. If you add a new write path, give it an
`isFatal` that covers every error that will never come good.

**Deletions must be idempotent-safe.** The watched ledger (`src/seen.js`) exists because a
delete without a memory becomes a delete/re-add loop that costs 100 units per video per
night, forever. Any change to add/remove logic must keep that guarantee: a video the ledger
knows is watched is never re-added, under any flag combination.

**Prefer a dry run and a small `--max`.** When testing against the live account, use
`--dry-run` first, then `--max=1`/`--max-removals=1`. Do not exercise a change by running
a full sync.

**Read before writing, literally.** Phase 1 reads the state of every channel before any
write happens, so a mid-run quota stop still produces an accurate report. Keep that shape —
do not interleave reads and writes for convenience.

## 12. This Repo Is Public

Treat everything tracked here as world-readable, because it is.

- **No addresses, account names, tenant ids, project ids, playlist ids or channel lists in
  tracked files.** Recipients, the account label and the channel list are configuration:
  `.env` and `config/channels.txt`, both git-ignored. `.env.example` and
  `config/channels.example.txt` are the tracked templates and carry `example.com`
  placeholders only.
- **No defaulting to a real address in code.** If configuration is missing, fail with a
  message naming the variable to set — never fall back to someone's mailbox.
- **Logs, `state/` and `report.html` stay ignored.** They contain video ids, channel
